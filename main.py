...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    welcome_message = f"""🎯 Добро пожаловать в {COMPANY_INFO['name']}!
...
def get_main_menu() -> InlineKeyboardMarkup:
    ...
    # Дроп сервис - открывает меню
    keyboard.append([InlineKeyboardButton(
        text="📦 Дроп сервис",
        callback_data="service_drop_service"
    )])
    
    # Отрисовка - прямая ссылка на менеджера
    keyboard.append([InlineKeyboardButton(
        text="🎨 Отрисовка",
        url="https://t.me/risujudljatebja"
    )])
    
    # Скуп товаров - прямая ссылка на менеджера
    keyboard.append([InlineKeyboardButton(
        text="💰 Скуп товаров",
        url="https://t.me/PROFIT_ASD"
    )])
    
    # Услуги Boxera - прямая ссылка на менеджера
    keyboard.append([InlineKeyboardButton(
        text="📦 Услуги Boxera",
        url="https://t.me/ASDSDA88"
    )])
    
    # Обучение - прямая ссылка на менеджера
    keyboard.append([InlineKeyboardButton(
        text="📚 Обучение",
        url="https://t.me/PROFIT_ASD"
    )])
...
{service.get('description', '')}"""
            ...
            # Добавляем кнопку Прайс ТОЛЬКО для Дроп сервиса
